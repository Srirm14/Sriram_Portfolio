...
function AbstractIllustration({
  ...
  const from = item.gradient.from;
  const via = item.gradient.via;
  const to = item.gradient.to;
  const id = item.id;
  ...
  const patterns: Record<string, React.ReactNode> = {
    delta: (
      <svg viewBox="0 0 280 120" fill="none"
        ...
        <circle cx="140" cy="60" r="40" stroke={from} strokeWidth="0.5"
          strokeDasharray="4 6" opacity="0.2" />
        <circle cx="140" cy="60" r="20" fill={from} opacity="0.08" />
        <circle cx="140" cy="60" r="6" fill={from} opacity="0.3" />
        <line x1="0" y1="60" x2="280" y2="60" stroke={via}
        ...
        {Array.from({ length: 5 }).map((_, row) =>
          Array.from({ length: 12 }).map((_, col) => (
            <circle key={`${row}-${col}`} cx={12 + col * 24} cy={14 + row * 24}
              r={1} fill={from} opacity={0.12 + (row + col) * 0.02} />
          ))
        )}
        <path d="M20 100 L70 40 L120 80 L170 30 L260 70"
          stroke={from} strokeWidth="1" opacity="0.35" fill="none"
      ...
      <svg viewBox="0 0 280 120" fill="none"
        ...
        {Array.from({ length: 6 }).map((_, row) =>
          Array.from({ length: 10 }).map((_, col) => (
            <circle key={`${row}-${col}`} cx={20 + col * 26} cy={16 + row * 18}
              r={isDev ? 1.5 : 1} fill={from}
              opacity={0.15 + (row + col) * 0.02} />
          ))
        )}
        <line x1="0" y1="120" x2="280" y2="0" stroke={from}
          strokeWidth="0.5" opacity="0.2" />
        <line x1="40" y1="120" x2="280" y2="20" stroke={via}
          strokeWidth="0.5" opacity="0.15" />
        <rect x="0" y="52" width="280" height="18" fill={from} opacity="0.06" />
        <path d="M8 8 L8 24 M8 8 L24 8" stroke={from} strokeWidth="1.5"
          opacity="0.5" strokeLinecap="round" />
        <path d="M272 112 L272 96 M272 112 L256 112" stroke={to}
        ...
        <defs>
          <linearGradient id={`scan-${id}`} x1="0" y1="0" x2="1" y2="0">
            ...
            <stop offset="50%" stopColor={from} stopOpacity="1" />
    ...
    calicalc: (
      <svg viewBox="0 0 280 120" fill="none"
        ...
        <line x1="0" y1="119" x2="280" y2="119"
          stroke={from} strokeWidth="1" opacity="0.2" />
        <circle cx="200" cy="26" r="3" fill={from} opacity="0.6">
          <animate attributeName="opacity" values="0.6;1;0.6"
            dur="2s" repeatCount="indefinite" />
        </circle>
        <line x1="200" y1="26" x2="200" y2="119"
          stroke={from} strokeWidth="0.5"
    ...
    uniwiz: (
      <svg viewBox="0 0 280 120" fill="none"
        ...
        <line x1="140" y1="60" x2="40" y2="30"
          stroke={from} strokeWidth="0.8" opacity="0.25" />
        <line x1="140" y1="60" x2="240" y2="30"
          stroke={from} strokeWidth="0.8" opacity="0.25" />
        <line x1="140" y1="60" x2="40" y2="90"
          stroke={via} strokeWidth="0.8" opacity="0.2" />
        <line x1="140" y1="60" x2="240" y2="90"
          stroke={via} strokeWidth="0.8" opacity="0.2" />
        <line x1="140" y1="60" x2="140" y2="15"
          stroke={to} strokeWidth="0.8" opacity="0.2" />
        <line x1="140" y1="60" x2="80" y2="105"
          stroke={to} strokeWidth="0.8" opacity="0.15" />
        <line x1="140" y1="60" x2="200" y2="105"
          stroke={to} strokeWidth="0.8" opacity="0.15" />
        {[[40,30], [240,30], [40,90], [240,90], [140,15], [80,105], [200,105]].map(
          ([cx, cy], i) => (
            <circle key={i} cx={cx} cy={cy} r={4}
              fill={i % 2 === 0 ? from : via} opacity={0.3} />
          )
        )}
        <circle cx="140" cy="60" r="10" fill={from} opacity="0.12" />
        <circle cx="140" cy="60" r="5" fill={from} opacity="0.4">
          <animate attributeName="r" values="5;7;5"
            dur="3s" repeatCount="indefinite" />
          <animate attributeName="opacity" values="0.4;0.2;0.4"
            dur="3s" repeatCount="indefinite" />
        </circle>
        <circle cx="140" cy="60" r="30" stroke={from} strokeWidth="0.5"
      ...
      <svg viewBox="0 0 280 120" fill="none"
        ...
        {[20,40,60,80,100].map(y => (
          <line key={y} x1="0" y1={y} x2="280" y2={y}
            stroke={from} strokeWidth="0.3" opacity="0.1" />
        ))}
        ...
        <path
          ...
          stroke={from} strokeWidth={isDev ? "1.5" : "2"} opacity="0.5"
          fill="none" strokeLinecap="round" strokeLinejoin="round"
        />
        <circle cx="65" cy="30" r="3" fill={from} opacity="0.6">
          <animate attributeName="opacity" values="0.6;1;0.6"
            dur="1.5s" repeatCount="indefinite" />
        </circle>
        <circle cx="165" cy="30" r="3" fill={from} opacity="0.4">
        ...
        <defs>
          <linearGradient id={`scan-neu-${id}`} x1="0" y1="0" x2="1" y2="0">
            <stop offset="0%" stopColor={from} stopOpacity="0" />
            <stop offset="50%" stopColor={from} stopOpacity="0.8" />
            <stop offset="100%" stopColor={from} stopOpacity="0" />
          </linearGradient>
        </defs>
      </svg>
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{
          background: isDev
            ? "radial-gradient(ellipse at center, transparent 40%, rgba(13,13,20,0.6) 100%)"
      ...
      <div className="absolute bottom-0 left-0 right-0 h-8 pointer-events-none"
        style={{
          background: isDev
            ? "linear-gradient(to bottom, transparent, rgba(13,13,20,0.95))"
            : "linear-gradient(to bottom, transparent, rgba(10,10,10,0.98))",
        }}
      />
    </div>
  );
    ...
    <motion.div
      ...
      style={
        isDev
          ...
          : {
              background: "#0a0a0a",
              boxShadow: "4px 4px 0px rgba(57,255,20,0.25)",
            }
      }
    >
        ...
        <div
          ...
          style={{
            background: `radial-gradient(ellipse at 50% 100%,
              ${item.gradient.from}12 0%, transparent 70%)`,
          }}
        />
          ...
          <a
            ...
            className={cn(
              ...
              isDev
                ...
                : "border border-[#39FF14]/40 text-[#39FF14]/80 hover:bg-[#39FF14]/20 hover:text-[#39FF14]",
            )}
          >
            {getDomain(item.link)}
      ...
      >
        <div className="flex items-baseline justify-between gap-2">
          <h3
            className={cn(
              ...
              isDev ? "font-grotesk font-bold" : "font-bebas tracking-wider uppercase text-base",
            )}
          >
            {item.title}
          </h3>
          <span
            className={cn(
              ...
              isDev ? "text-white/20" : "text-[#39FF14]/25",
            )}
          >
        ...
        <p
          className={cn(
            ...
            isDev ? "font-poppins text-white/40" : "font-mono text-white/35",
          )}
        >
          {item.shortDesc}
        </p>
          ...
          {(isDev ? item.devTech : item.designTools).length > 3 && (
            <span
              className={cn(
                ...
                isDev ? "text-white/20" : "text-[#39FF14]/30",
              )}
            >
              +{(isDev ? item.devTech : item.designTools).length - 3}
            </span>
      ...
      <div
        ...
        style={{
          background: isDev
            ? `linear-gradient(90deg, ${item.gradient.from}80, ${item.gradient.to}40, transparent)`
            : "rgba(57,255,20,0.4)",
        }}
      />
    </motion.div>
